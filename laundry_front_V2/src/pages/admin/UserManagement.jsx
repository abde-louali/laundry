...
const UserManagement = () => {
  ...
  const handleToggleStatus = (user, isActive) => {
    setConfirmModal({
      ...
      message: `L'utilisateur ${user.name} ${isActive ? 'sera suspendu' : 'pourra à nouveau accéder au système'}.`,
        ...
        <div className="hidden md:block p-6 border-b border-border space-y-4 bg-background/30">
          <div className="flex items-center justify-between">
            <div>
                ...
                <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest">{currentUsers.length} au total</p>
            </div>
            <button
          ...
          <div className="flex border-b border-border">
            {[
                { key: 'active', label: 'Actifs', count: activeUsers?.length || 0 },
                { key: 'inactive', label: 'En attente/Inactifs', count: inactiveUsers?.length || 0 }
            ].map(t => (
              <button 
                key={t.key} 
                onClick={() => setActiveTab(t.key)}
                className={`flex-1 pb-3 text-xs font-bold transition-all relative ${activeTab === t.key ? 'text-primary-600' : 'text-text-muted hover:text-text-secondary'}`}
              >
                {t.label} 
                <span className="ml-1.5 font-medium">({t.count})</span>
                {activeTab === t.key && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary-500 rounded-full" />}
              </button>
            ))}
          </div>
        </div>
              ...
              return (
                <div key={user.id} className="md:contents">
                  ...
                  <button 
                      ...
                      className={`hidden md:flex w-full items-center gap-4 p-4 text-left transition-all ${isSel ? 'bg-primary-50/50' : 'hover:bg-background/50'}`}
                  >
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-base shadow-sm shrink-0 transition-transform ${isSel ? 'scale-110' : ''} ${activeTab === 'active' ? 'bg-white border border-border text-primary-600' : 'bg-gray-100 text-text-muted'}`}>
                  ...
                  {/* Mobile User Card */}
                  <div className="md:hidden bg-white rounded-2xl shadow-card p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-14 h-14 rounded-full bg-primary-100 text-primary-600 flex items-center justify-center text-xl font-bold shadow-sm">
                          {initials(user.name)}
                        </div>
                        <div>
                          ...
                          <div className="flex items-center mt-1">
                            ...
                            <span className="text-xs text-text-muted ml-2">ID: PC-{user.id?.toString().padStart(4, '0')}</span>
                          </div>
                        </div>
                      </div>
                      <div 
                        onClick={() => handleToggleStatus(user, activeTab === 'active')}
                        className={`w-12 h-6 rounded-full relative cursor-pointer transition-all duration-200 ${activeTab === 'active' ? 'bg-primary-500' : 'bg-gray-200'}`}
                      >
                        <div className={`w-5 h-5 bg-white rounded-full shadow absolute top-0.5 transition-all duration-200 ${activeTab === 'active' ? 'right-0.5' : 'left-0.5'}`} />
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                {!isAddingUser && selectedUser && (
                   <div className="bg-surface rounded-3xl shadow-card p-8 border border-border/50 flex flex-col items-center justify-center text-center">
                        <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-6 shadow-sm border ${activeTab === 'active' ? 'bg-orange-50 text-orange-500 border-orange-100' : 'bg-green-50 text-green-500 border-green-100'}`}>
                            <Power size={32} />
                        </div>
                        ...
                        <div className="w-full flex flex-col gap-3">
                            <button
                                ...
                                className={`w-full flex items-center justify-center gap-2 px-6 py-3 rounded-2xl text-xs font-black transition-all ${activeTab === 'active' ? 'bg-orange-600 text-white shadow-lg shadow-orange-500/20 hover:bg-orange-700' : 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/20 hover:bg-emerald-700'}`}
                            >
                                <Power size={18} />
                                {activeTab === 'active' ? 'SUSPENDRE ACCÈS' : 'RÉTABLIR ACCÈS'}
                            </button>
                            ...
                            {activeTab === 'inactive' && (
                                <button
                                    onClick={() => setConfirmModal({ 
                                        isOpen: true, 
                                        title: 'DANGER: Suppression Définitive', 
                                        message: `Cette action supprimera ${selectedUser.name} de la base de données de manière irréversible. Toutes les données liées seront conservées mais l'utilisateur disparaîtra.`, 
                                        type: 'danger', 
                                        onConfirm: () => { dispatch(removeUser(selectedUser.id)); setSelectedUser(null); setConfirmModal(p => ({ ...p, isOpen: false })); } 
                                    })}
